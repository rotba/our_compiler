...
push rax
push 1
;(LambdaSimple'(  ( fact ) , [(Seq'( [(Set'( Var' ( VarParam ( fact , 0 )) , (Box'( VarParam ( fact , 0 ) ) ) , (Seq'( [(BoxSet'( VarParam ( fact , 0 ) , (LambdaSimple'(  ( n ) , [(If'( Applic'( Var' ( VarFree ( = )) , [Var' ( VarParam ( n , 0 )) , Const' ( 0)] ) , Const' ( 1),ApplicTP'( Var' ( VarFree ( * )) , [Var' ( VarParam ( n , 0 )) , Applic'( (BoxGet'( VarBound ( fact , 0 , 0 ) ) , [Applic'( Var' ( VarFree ( - )) , [Var' ( VarParam ( n , 0 )) , Const' ( 1)] )] )] ) )] ) ) , ApplicTP'( (BoxGet'( VarParam ( fact , 0 ) ) , [Const' ( 2)] )] )] )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
...
shr rcx, 3
cmp rcx, 0
je no_more_params_3600
params_loop_3599:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_3599
no_more_params_3600:
mov qword[rdx], rbx
is_empty_3597:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_3601)
jmp Lcont_3602
Lcode_3601:
push rbp
mov rbp, rsp
