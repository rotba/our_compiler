...
push 0
;(LambdaSimple'(  (  ) , [ApplicTP'( (LambdaOpt'(  ( x . y ), , [ApplicTP'( Var' ( VarFree ( car )) , [Var' ( VarParam ( y , 1 ))] )] ) , [Const' ( 1) , Const' ( 2)] )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
jne is_not_empty_395
MALLOC rdx, 8
mov qword[rdx], SOB_NIL_ADDRESS
jmp is_empty_396
is_not_empty_395:
...
shr rcx, 3
cmp rcx, 0
je no_more_params_399
params_loop_398:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_398
no_more_params_399:
mov qword[rdx], rbx
is_empty_396:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_400)
jmp Lcont_401
Lcode_400:
push rbp
mov rbp, rsp
...
push rax
push 2
;(LambdaOpt'(  ( x . y ), , [ApplicTP'( Var' ( VarFree ( car )) , [Var' ( VarParam ( y , 1 ))] )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
jne is_not_empty_417
MALLOC rdx, 8
mov qword[rdx], SOB_NIL_ADDRESS
jmp is_empty_418
is_not_empty_417:
...
shr rcx, 3
cmp rcx, 0
je no_more_params_421
params_loop_420:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_420
no_more_params_421:
mov qword[rdx], rbx
is_empty_418:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_422)
jmp Lcont_423
Lcode_422:
;donte the effective numer of parameters m
;donte the noumber of simple parameters n
