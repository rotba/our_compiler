...
push rax
push 1
;(LambdaSimple'(  ( x ) , [(If'( Applic'( (LambdaSimple'(  (  ) , [Var' ( VarBound ( x , 0 , 0 ))] ) , [] ) , Const' ( 2),Const' ( 3) )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
...
shr rcx, 3
cmp rcx, 0
je no_more_params_416
params_loop_415:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_415
no_more_params_416:
mov qword[rdx], rbx
is_empty_413:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_417)
jmp Lcont_418
Lcode_417:
push rbp
mov rbp, rsp
