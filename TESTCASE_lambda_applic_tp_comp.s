...
push rax
push 1
;(LambdaSimple'(  ( x ) , [ApplicTP'( Var' ( VarFree ( + )) , [Var' ( VarParam ( x , 0 )) , Const' ( 2)] )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
jne is_not_empty_400
MALLOC rdx, 8
mov qword[rdx], SOB_NIL_ADDRESS
jmp is_empty_401
is_not_empty_400:
...
shr rcx, 3
cmp rcx, 0
je no_more_params_404
params_loop_403:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_403
no_more_params_404:
mov qword[rdx], rbx
is_empty_401:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_405)
jmp Lcont_406
Lcode_405:
push rbp
mov rbp, rsp
