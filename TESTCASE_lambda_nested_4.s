...
push rax
push 1
;(LambdaSimple'(  ( x ) , [ApplicTP'( (LambdaSimple'(  ( y ) , [Var' ( VarBound ( x , 0 , 0 ))] ) , [Const' ( 2)] )] )
GET_ENV rbx
mov rcx, 0
cmp rbx, SOB_NIL_ADDRESS
...
shr rcx, 3
cmp rcx, 0
je no_more_params_411
params_loop_410:
mov rdi, rcx
dec rdi;rdi is the 0 based index of the current arg
GET_ARG rsi, rdi
mov qword[rbx + rdi*8], rsi
loop params_loop_410
no_more_params_411:
mov qword[rdx], rbx
is_empty_408:
;;RDX IS THE EXTENV!!!
MAKE_CLOSURE(rax, rdx, Lcode_412)
jmp Lcont_413
Lcode_412:
push rbp
mov rbp, rsp
